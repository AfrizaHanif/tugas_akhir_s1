...
@if (Request::is('admin/inputs/votes') || Request::is('officer/votes'))
    ...
    <div class="modal modal-xl fade" id="modal-chk-view-{{ $latest_per->id_period }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Cek Pegawai ({{ $latest_per->name }})</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @foreach ($criterias as $criteria)
                                    <td>
                                        @if ($checks->where('id_officer', $officer->id_officer)->where('id_period', $latest_per->id_period)->where('id_vote_criteria', $criteria->id_vote_criteria)->count() == 0)
                                    ...
                                    @endforeach
                                    <td>
                                        @if ($checks->where('id_officer', $officer->id_officer)->where('id_period', $latest_per->id_period)->count() == 0)
                                        <span class="badge text-bg-danger">Belum Memilih</span>
                                        @elseif ($checks->where('id_officer', $officer->id_officer)->where('id_period', $latest_per->id_period)->count() < $criterias->count())
    ...
    <!--VOTE CHECKER (ALL)-->
    <div class="modal modal-xl fade" id="modal-chk-all-{{ $latest_per->id_period }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Cek Pegawai ({{ $latest_per->name }})</h1>
        ...
        @foreach ($criterias as $criteria)
            ...
            <!--SELECT VOTE-->
            <div class="modal fade" id="modal-vte-select-{{ $latest_per->id_period }}-{{ $vote->id_officer }}-{{ $criteria->id_vote_criteria }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        @if (Auth::check() && Auth::user()->part != 'Pegawai')
                        <form action="{{ route('admin.inputs.votes.select', ['period'=>$latest_per->id_period, 'officer'=>$vote->id_officer, 'criteria'=>$criteria->id_vote_criteria]) }}" method="POST" enctype="multipart/form-data">
                        @else
                        <form action="{{ route('officer.votes.select', ['period'=>$latest_per->id_period, 'officer'=>$vote->id_officer, 'criteria'=>$criteria->id_vote_criteria]) }}" method="POST" enctype="multipart/form-data">
                        @endif
                            <div class="modal-header">
