                ...
                <div class="row g-2">
                    ...
                    <div class="col-md-4">
                        <div class="position-sticky" style="top: 0rem;">
                            <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                ...
                                <button class="nav-link {{ $loop->first ? 'active' : '' }} text-start" id="pills-{{ $hyear->period_year }}-tab" data-bs-toggle="pill" data-bs-target="#pills-{{ $hyear->period_year }}" type="button" role="tab" aria-controls="pills-{{ $hyear->period_year }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">
                                    <i class="bi bi-check-lg"></i> {{ $hyear->period_year }}
                                </button>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="col-md-8">
                        <div class="tab-content" id="v-pills-tabContent">
                            ...
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pills-{{ $hyear->period_year }}" role="tabpanel" aria-labelledby="pills-{{ $hyear->period_year }}-tab" tabindex="0">
                                <h2>{{ $hyear->period_year }}</h2>
                                <table class="table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($hscores->where('id_officer', Auth::user()->nip)->where('period_year', $hyear->period_year) as $score)
                                        <tr>
