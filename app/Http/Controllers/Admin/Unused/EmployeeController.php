...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($latest_per)){
            if($latest_per->progress_status == 'Verifying'){ //PROGRESS: VERIFYING
                ...
                //RETURN TO VIEW
                return redirect()->route('admin.masters.employees.index')->with('fail','Tidak dapat menambahkan karyawan dikarenakan sedang dalam proses verifikasi nilai.')->with('code_alert', 2)->withInput(['tab_redirect'=>'pills-'.$request->id_part])->with('modal_redirect', 'modal-emp-create');
            }
        }
        ...
        */
        ...
        $count_lead = Employee::with('position')->whereHas('position', function($query){$query->where('name', 'LIKE', 'Kepala%');})
        ...
        if($request->id_sub_team_1 == $request->id_sub_team_2){
            ...
            //RETURN TO VIEW
            return redirect()
            ->route('admin.masters.employees.index')->with('fail','Tim Utama dan Tim Cadangan tidak boleh sama. Jika hanya satu karyawan, pilih Tidak Ada di Tim Cadangan.')
    ...
    public function update(Request $request, Employee $employee)
    {
        ...
        */
        ...
        if(!empty(User::where('nip', $employee->id_employee)->first())){ //IF USER EXISTS
            User::where('nip', $employee->id_employee)->update([
                'name'=>$request->name,
                'nip'=>$request->id_employee,
                'part'=>$part,
            ]);
    ...
    public function destroy(Employee $employee)
    {
        ...
        //DESTROY DATA
        if(!empty(User::where('nip', $employee->id_employee)->first())){
    ...
    public function import(Request $request): RedirectResponse
    {
        ...
        try{
            ...
            if($errorCode == 1062){ //IF DATA IN EXCEL DUPLICATE
                ...
                if(Auth::user()->part == 'Dev'){
                    return redirect()
                    ->route('developer.employees.index')
