...
class OfficerController extends Controller
{
    public function index()
    {
        ...
        if(Auth::check()){
            if(Auth::user()->part == "Admin" || Auth::user()->part == "KBPS"){
                return view('Pages.Admin.officer', compact('parts', 'parts_2', 'positions', 'kbps_positions', 'umum_positions', 'off_positions', 'teams', 'team_lists', 'subteams', 'officers', 'users'));
                ...
                return view('Pages.Officer.officer', compact('parts', 'parts_2', 'positions', 'kbps_positions', 'umum_positions', 'off_positions', 'teams', 'team_lists', 'subteams', 'officers', 'users'));
            }
        }else{
            return view('Pages.Home.officer', compact('parts', 'parts_2', 'positions', 'kbps_positions', 'umum_positions', 'off_positions', 'teams', 'team_lists', 'subteams', 'officers', 'users', 'officers'));
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($latest_per)){
            if($latest_per->progress_status == 'Verifying'){ //PROGRESS: VERIFYING
                ...
                //RETURN TO VIEW
                return redirect()->route($redirect_route)->with('fail','Tidak dapat menambahkan pegawai dikarenakan sedang dalam proses verifikasi nilai.')->with('code_alert', 2)->withInput(['tab_redirect'=>'pills-'.$request->id_part])->with('modal_redirect', 'modal-off-create');
            }
        }
        ...
        */
        ...
        if($request->id_sub_team_1 == $request->id_sub_team_2){
            ...
            //RETURN TO VIEW
            return redirect()
            ->route($redirect_route)->with('fail','Tim Utama dan Tim Cadangan tidak boleh sama. Jika hanya satu pegawai, pilih Tidak Ada di Tim Cadangan.')
        ...
        //AUTO DETECT POSITION (FOR FUTURE DEVELOPMENT)
        /*
    ...
    public function update(Request $request, Officer $officer)
    {
        ...
        if($request->filled('id_sub_team_1')){
            ...
            if($request->id_position == 'DPT-002'){
        ...
        */
        ...
        if(!empty(User::where('nip', $officer->id_officer)->first())){ //IF USER EXISTS
    ...
    public function search(Request $request)
    {
        ...
        if(Auth::check()){
            if(Auth::user()->part == "Admin" || Auth::user()->part == "KBPS"){
                ...
                return view('Pages.Officer.officer', compact('parts', 'positions', 'teams', 'subteams', 'officers', 'search'));
            }
        }else{
