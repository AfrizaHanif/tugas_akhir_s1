...
class VoteController extends Controller
{
    public function index()
    {
        ...
        if(Auth::user()->part != "Pegawai"){
            return view('Pages.Admin.vote', compact('periods', 'votes', 'officers', 'checks', 'fil_offs'));
        }else{
            return view('Pages.Home.vote', compact('periods', 'votes', 'officers', 'checks'));
        }
    }

    public function select($period, $officer)
    {
        //INCREMENT DATA
        Vote::where('id_period', $period)->where('id_officer', $officer)->increment('votes');
        ...
        //RETURN TO VIEW
        if(Auth::user()->part != "Pegawai"){
            return redirect()->route('inputs.votes.index')->with('success','Voting Berhasil');
        }else{
            return redirect()->route('votes.index')->with('success','Voting Berhasil');
        }
    }
}
